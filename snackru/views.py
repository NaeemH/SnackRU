...
from snackru import app, lm, db, mail
...
def oauth_callback(provider):
    if not current_user.is_anonymous:
        return redirect(url_for('index'))
    oauth = OAuthSignIn.get_provider(provider)
    snackru_id, name, email = oauth.callback()
    if snackru_id is None:
        flash('Authentication failed.')
        return redirect(url_for('index'))
    user = User.query.filter_by(snackru_id=snackru_id).first()
    if not user: # Create, add and login new user. Redirect to /register
        user = User(snackru_id=snackru_id, name=name, email=email)
        db.session.add(user)
        db.session.commit()
